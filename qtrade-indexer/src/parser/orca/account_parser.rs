...
// qtrade: from account_helpers.rs in raydium_clmm
use spl_pod::solana_pubkey::Pubkey;

// For help in naming spans
use crate::QTRADE_INDEXER_TRACER_NAME;
const ORCA_PROGRAM_STATE: &str = "orca::OrcaProgramState";
const ORCA_ACCOUNT_PARSER: &str = "orca::AccountParser";

#[derive(Debug, Clone, BorshDeserialize, BorshSerialize)]
pub struct KeyedWhirlpool {
    pub pubkey: Pubkey,
    pub whirlpool: Whirlpool,
}

#[derive(Debug, Clone, BorshDeserialize, BorshSerialize)]
pub struct KeyedWhirlpoolsConfig {
    pub pubkey: Pubkey,
    pub whirlpools_config: WhirlpoolsConfig,
}
...
impl OrcaProgramState {
    pub fn try_unpack(pubkey_bytes: [u8; 32], data_bytes: &[u8]) -> ParseResult<Self> {
        ...
        let span_name = format!("{}::try_unpack", ORCA_PROGRAM_STATE);
        ...
        let result = match data_len {
            ...
            */
            Whirlpool::LEN => {
                let whirlpool = Whirlpool::from_bytes(data_bytes)?;
                Ok(OrcaProgramState::Whirlpool(KeyedWhirlpool {
                    pubkey,
                    whirlpool,
                }))
            },
...
impl Parser for AccountParser {
    ...
    async fn parse(&self, acct: &AccountUpdate) -> ParseResult<Self::Output> {
        let tracer = global::tracer(QTRADE_INDEXER_TRACER_NAME);
        let span_name = format!("{}::parse", ORCA_ACCOUNT_PARSER);
        ...
        let result = tracer.in_span(span_name, |_cx| async move {
            ...
            OrcaProgramState::try_unpack(pubkey_bytes, &inner.data)
        }).await;
...
mod tests {
    ...
    async fn test_whirlpool_account_parsing() {
        ...
        /* qtrade TODO: get test working again/
