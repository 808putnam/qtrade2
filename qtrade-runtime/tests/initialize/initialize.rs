...
use raydium_amm_v3_client::*;
// use raydium_amm_v3_client::instructions::token_instructions::*;
// use raydium_amm_v3_client::instructions::rpc::*;
...
fn start_validator() {
    ...
    let pool_config = raydium_amm_v3_client::instructions::load_cfg(&client_config.to_string()).unwrap();
    // cluster params.
    let payer = raydium_amm_v3_client::instructions::read_keypair_file(&pool_config.payer_path).expect("Failed to load pool_config.payer_path");
    // solana rpc client
...
fn create_mint(mint_key_file: &str, mint_authority_file: &str, pool_config: &raydium_amm_v3_client::instructions::ClientConfig) {
    let mint_keypair = instructions::read_keypair_file(mint_key_file)
        .expect(&format!("Failed to read {} keypair", mint_key_file));
    let mint_key = mint_keypair.pubkey();
    let mint_authority_keypair = instructions::read_keypair_file(mint_authority_file)
        .expect(&format!("Failed to read {} keypair", mint_authority_file));
    let mint_authority = mint_authority_keypair.pubkey();
    let instructions = raydium_amm_v3_client::instructions::token_instructions::create_and_init_mint_instr(
        pool_config,
        spl_token::id(),
        &mint_key,
        &mint_authority,
    ...
    let signature = raydium_amm_v3_client::instructions::rpc::send_txn(&rpc_client, &txn, true).expect("Failed to send transaction");
