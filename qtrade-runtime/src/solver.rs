...
// Import Cache trait and PoolCacheState
use crate::POOL_CACHE;
use crate::streamer::Cache; // Import the Cache trait for method access
...
lazy_static! {
    pub static ref ARBITRAGE_SENDER: tokio::sync::Mutex<mpsc::Sender<ArbitrageResult>> = {
        ...
        // We'll expose this through a function that lander.rs can call
        crate::lander::init_arbitrage_receiver(rx);
