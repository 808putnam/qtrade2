#!/bin/bash
# Changing from the original as we already have a bare-metal server with
# a formatted nvme0n1 disk

# Set the minimum required disk size in terabytes (1.8TB for nvme1n1)
MIN_SIZE=1.8

# Find all disks using lsblk, exclude the OS disk (nvme0n1), and filter by size
disks=$(lsblk -d -n -o NAME,SIZE | grep -vE 'loop|sr|nvme0n1' | awk -v min_size=$MIN_SIZE '$2 ~ /T$/ && substr($2, 1, length($2)-1) >= min_size {print $1}')

# Convert to an array and check if we have at least 2 suitable disks
IFS=$'\n' read -r -d '' -a disk_array <<< "$disks"

echo "Disks to be processed: ${disk_array[@]}"

# Leaving comment for reference for future bare-metal server setups
# if [ "${#disk_array[@]}" -lt 2 ]; then
#    echo "Error: Less than 2 disks with at least ${MIN_SIZE}TB of space were found."
#    exit 1
# fi
...
mkdir -p /home/ubuntu/Solana/log
...
# Process the additional disk
# Leaving commented line in for reference for future bare-metal server setups
# partition_and_mount_disk "${disk_array[0]}" "$MOUNT_POINT1"
partition_and_mount_disk "${disk_array[0]}" "$MOUNT_POINT2"
