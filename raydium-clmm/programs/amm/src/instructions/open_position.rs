...
// qtrade
// removed here and below as is a legacy on-chain program
...
fn mint_nft_and_remove_mint_authority<'info>(
    ...
    _personal_position: &Account<'info, PersonalPositionState>,
    position_nft_mint: &AccountInfo<'info>,
    position_nft_account: &AccountInfo<'info>,
    _metadata_account: Option<&UncheckedAccount<'info>>,
    _metadata_program: Option<&Program<'info, Metadata>>,
    ...
    // qtrade
    // also adjusted fields not used above
    // removed here as:
    // 1. There is a call to spl-token-2022 that we haven't figured out yet
    // 2. This has calls to a legacy on-chain program
    // This has an effect on the api open_position_with_token22_nft in that you will need
    // to set with_metadata to false for now.
    /*
    if with_metadata {
...
// qtrade
// removed here as this has calls to a legacy on-chain program
/*
